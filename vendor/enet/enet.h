...
ENET_API void			     enet_host_use_proxy(ENetHost*, const char*, uint16_t, const char*, const char*);
...
void enet_host_use_proxy(ENetHost* host, const char* ip, uint16_t port, const char* username, const char* password)
{
    host->usingProxy = true;
    char* paddress = new char[strlen(ip) + 1];
    char* pusername = new char[strlen(username) + 1];
    char* ppassword = new char[strlen(password) + 1];
...
enet_uint32 enet_peer_get_ping(ENetPeer* peer)
{
    return peer->roundTripTime;
}
#pragma warning(pop)
